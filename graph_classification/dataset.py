...
from utils import compute_ppr, normalize_adj, compute_lapla
from numpy.linalg import eig, eigh
import torch

def download(dataset):
    basedir = os.path.dirname(os.path.abspath(__file__))
    datadir = os.path.join(basedir, '/home/zjk/Graph/data', dataset)
...
def process(dataset):
    src = os.path.join(os.path.dirname(__file__), '/home/zjk/Graph/data')
    ...
    for idx in range(1, 1 + len(adj_list)):
        ...
        lapla.append(compute_lapla(graph))
...
def load(dataset):
    ...
    if not os.path.exists(datadir):
        download(dataset)
        graphs, diff, lap = process(dataset)
        feat, adj, labels = [], [], []
        ...
        adj, diff, lap, feat, labels = np.array(adj), np.array(diff), np.array(lap), np.array(feat), np.array(labels)

        np.save(f'{datadir}/adj.npy', adj)
        np.save(f'{datadir}/diff.npy', lap)
        np.save(f'{datadir}/lap.npy', lap)
        ...
        lap = np.load(f'{datadir}/lap.npy', allow_pickle=True)
    ...
    for idx in range(adj.shape[0]):
        ...
        E[idx] = np.hstack((E[idx], np.zeros(max_nodes-adj[idx].shape[0])))
        U[idx] = np.hstack(
            (np.vstack((U[idx], np.zeros((max_nodes - adj[idx].shape[0], adj[idx].shape[0])))),
             np.zeros((max_nodes, max_nodes - adj[idx].shape[0]))))
    ...
    U = np.array(np.array(U).tolist()).reshape(-1, max_nodes, max_nodes)
    E = np.array(np.array(E).tolist()).reshape(-1, max_nodes)
    


    return adj, diff, feat, labels, num_nodes, E, U

if __name__ == '__main__':
    # MUTAG, PTC_MR, IMDB-BINARY, IMDB-MULTI, PROTEINS
    adj, diff, feat, labels, num_nodes, E, U = load('IMDB-BINARY')
    print('done')
